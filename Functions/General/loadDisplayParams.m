...
% work). What a mess!
% 07/20/13 load wavelength together with spectra
% 08/17/13 add fields to be compatible with ISET
...
try
    ...
    if(exist(fullfile(displayPath,'spectra.mat'),'file'))
        c = load(fullfile(displayPath,'spectra.mat'));
        if ~isfield(c,'monitorSpectra') || ~isfield(c,'wavelengths')
            disp('Spectra not in spectra file (only important for LMS-specified stimuli)');
        else
            displayID.spectra = c.monitorSpectra(:,1:3);
            displayID.wavelengths = c.wavelengths;
            disp('Loaded spectra.');
        end
    else
    ...
    % Add field to be compatible with ISET display
    displayID.bits = displayID.cmapDepth;
    displayID.type = 'display';
    displayID.name = displayID.monitor;
