function [deltaEImage, whiteXYZ] = plotBrettelThreshold(display, ...
                            refColor, bgColor, cbType, whiteXYZ, varargin)
%% function plotBrettelThreshold(display, refColor, [bgColor], ...
%                                        [cbType], [whiteXYZ], [varargin])
%    plot colorblind theoretical threshold in LM contrast plane around
%    reference color point. We might add funtionalities to enable it to
%    plot in other color space in the future.
%
%  Inputs:
...
%    cbType      - colorblind type, defaut 2 (i.e. deuteranope)
%                  1 - protanope, 2 - deuteranope, 3 - tritanope
...
%    plotBrettelThreshold(display, [0.5 0.5 0.5]);
%
%  See also:
...
refXYZ  = lms2xyz(xyz2lms(refXYZ, cbType, whiteXYZ));
...
stimXYZImage = lms2xyz(xyz2lms(stimXYZImage, cbType, whiteXYZ));
